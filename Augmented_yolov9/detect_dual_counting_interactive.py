...
def check_intersection(line1_start, line1_end, line2_start, line2_end):
    """
    Check if line segment (line1_start, line1_end) intersects with 
    line segment (line2_start, line2_end).
    """
    def ccw(A, B, C):
...
def get_line_from_first_frame(source):
    line_points = []

    def draw_line(event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            line_points[:] = [(x, y)]  # Update start point
        elif event == cv2.EVENT_LBUTTONUP:
            line_points.append((x, y))  # Add end point
    ...
    if source.endswith('.mp4') or source.endswith('.avi'):
        ...
        if not success:
            ...
            return None, None
        cap.release()
    else:  # Assume source is an image
    ...
    cv2.imshow("First Frame", frame)
    cv2.setMouseCallback("First Frame", draw_line)
    cv2.waitKey(0)
...
def draw_boxes(frame, bbox_xyxy, draw_trails, identities=None, categories=None, offset=(0,0),line_start=None,line_end=None,counter=None):
    ...
    if line_start and line_end:
        cv2.line(frame, line_start, line_end, (0, 255, 0), thickness=2) # Green Line
    ...
    for i, box in enumerate(bbox_xyxy):
        ...
        if line_start and line_end and counter is not None and len(data_deque[id]) > 1:
            prev_center = data_deque[id][1]
            current_center = data_deque[id][0]

            # Check if the line formed by prev_center and current_center intersects with the drawn line
            if check_intersection(prev_center, current_center, line_start, line_end):
                counter['count'] += 1
        
        if counter['count'] != 0 :
            # Position for the text
            position = (frame.shape[1] - 300, 50)  # Adjust the x-value to move it left or right, and the y-value to move it up or down
            ...
            line_type = 2
            # Overlay the text on the top right corner of the frame
            cv2.putText(frame, f"Crossings: {counter['count']}", position, font, font_scale, font_color, line_type)
...
def run(
        ...
        draw_trails = False,
        line_start=None,  # Default values as examples
        line_end=None,
        counter = {'count': 0},
        draw_line=False
):
    if draw_line is not False:
        line_start,line_end = get_line_from_first_frame(source)
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                if len(outputs) > 0:
                    ...
                    object_id = outputs[:, -1]
                    draw_boxes(ims, bbox_xyxy, draw_trails, identities, object_id,line_start=line_start,line_end=line_end,counter=counter)
...
def parse_opt():
    ...
    parser.add_argument('--line-start', nargs=2, type=int, default=None, help='Start point of the line (x, y)')
    parser.add_argument('--line-end', nargs=2, type=int, default=None, help='End point of the line (x, y)')
    parser.add_argument('--draw-line', action='store_true', help='Interactively draw a line on the first frame')
